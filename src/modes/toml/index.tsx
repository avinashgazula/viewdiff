...
function tomlDiff(left: unknown, right: unknown, key: string | number = 'root'): DiffNode {
  ...
  if (leftIsArray && rightIsArray) {
    ...
    const children = Array.from({ length: len }, (_, i) => tomlDiff(la[i], ra[i], i))
  ...
  if (!leftIsArray && !rightIsArray) {
    ...
    const children = [...allKeys].map((k) => tomlDiff(lo[k], ro[k], k))
    const anyDiff = children.some((c) => c.type !== 'same')
...
function TomlValue({ val }: { val: unknown }) {
  ...
  if (val instanceof Date) return <span style={{ color: '#56b6c2' }}>{val.toISOString()}</span>
  if (typeof val === 'string')
    return <span style={{ color: 'var(--text-secondary)' }}>{val.length > 100 ? val.slice(0, 100) + '…' : val}</span>
  return <span style={{ color: 'var(--text-dim)' }}>{JSON.stringify(val).slice(0, 60)}</span>
}

function TomlDiffRow({
  node, depth, showOnly, searchQuery, expandOverride, path,
}: {
  node: DiffNode
  depth: number
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px', paddingLeft: indent + 8, borderRight: '1px solid var(--border-subtle)', gap: 6 }}>
          ...
          <span style={{ fontSize: 11.5, fontFamily: 'var(--font-mono)' }}>
            {node.type !== 'added' ? <TomlValue val={node.leftVal} /> : <span style={{ color: 'var(--text-dim)', fontStyle: 'italic' }}>—</span>}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px', paddingLeft: 8, gap: 6 }}>
          ...
          <span style={{ fontSize: 11.5, fontFamily: 'var(--font-mono)' }}>
            {node.type !== 'removed' ? <TomlValue val={node.rightVal} /> : <span style={{ color: 'var(--text-dim)', fontStyle: 'italic' }}>—</span>}
          </span>
        </div>
      </div>
    )
...
export function TomlMode() {
  ...
  const diffRoot = useMemo(() => {
    ...
    return tomlDiff(leftParsed, rightParsed, 'root')
  ...
  const share = useCallback(async () => {
    ...
    await navigator.clipboard.writeText(`${window.location.origin}/toml?l=${encodeURIComponent(encL)}&r=${encodeURIComponent(encR)}`)
    setShareCopied(true)
  ...
  function exportReport() {
    if (!diffRoot) return
    const lines: string[] = ['TOML Diff Report', '================', '']
    ...
    <div className="h-screen flex flex-col" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      <nav className="toolbar" style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        ...
        <div className="toolbar-controls">
          {hasBoth && diffStats && (
            <>
              {diffStats.added > 0 && <button className="btn outlined" style={{ fontSize: 11, color: 'var(--green)' }}>+{diffStats.added}</button>}
              {diffStats.removed > 0 && <button className="btn outlined" style={{ fontSize: 11, color: 'var(--red)' }}>−{diffStats.removed}</button>}
              {diffStats.changed > 0 && <button className="btn outlined" style={{ fontSize: 11, color: 'var(--amber)' }}>≠{diffStats.changed}</button>}
              <button
          ...
          {hasBoth && (
            <>
              ...
              <button className="btn outlined" onClick={() => { setExpandKey((k) => k + 1); setExpandOverride(true) }}>Expand all</button>
              <button className="btn outlined" onClick={() => { setExpandKey((k) => k + 1); setExpandOverride(false) }}>Collapse all</button>
              <div className="divider" aria-hidden="true" />
              <input
                type="search"
                value={searchQuery}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--surface)', flexShrink: 0 }}>
                  ...
                  <label className="btn outlined" style={{ fontSize: 11.5, cursor: 'pointer' }}>
                    Open file
                    <input type="file" accept=".toml" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) f.text().then((t) => side === 'left' ? setLeftText(t) : setRightText(t)) }} />
                  </label>
                </div>
                <textarea
                  value={side === 'left' ? leftText : rightText}
                  onChange={(e) => side === 'left' ? setLeftText(e.target.value) : setRightText(e.target.value)}
                  placeholder="Paste TOML here or drop a .toml file…"
                  spellCheck={false}
                  style={{
        ...
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
          {!diffRoot && !leftError && !rightError ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8, flex: 1, padding: 40 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-muted)' }}>Paste TOML in both panels to compare</div>
              <div style={{ fontSize: 13, color: 'var(--text-dim)' }}>Supports Cargo.toml, pyproject.toml, config.toml, and any TOML file</div>
            </div>
          ) : (leftError || rightError) ? (
            <div style={{ padding: 20, color: 'var(--red)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
              {leftError ? `Original TOML error: ${leftError}` : `Modified TOML error: ${rightError}`}
            </div>
          ) : diffRoot ? (
            <div key={expandKey}>
              <div style={{
              ...
              <TomlDiffRow node={diffRoot} depth={0} showOnly={showOnly} searchQuery={searchQuery} expandOverride={expandOverride} path="root" />
            </div>
          ) : null}
        </div>
      </div>
      ...
      <div className="status-bar">
        ...
        <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>TOML structured diff</div>
      </div>
    </div>
  )
}
